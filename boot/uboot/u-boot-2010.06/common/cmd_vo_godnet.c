...
int check_vo_support(unsigned int dev, unsigned int type, unsigned int sync)
{
    ...
    {
        if ((type & (VO_INTF_VGA | VO_INTF_YPBPR))
            == (VO_INTF_VGA | VO_INTF_YPBPR))
        {
            printf("Hd%d can't use VGA and YPBPR at the same time!\n", dev);
            return -1;
        }

        if ((type & VO_INTF_BT656) && (type != VO_INTF_BT656))
        {
            printf("Hd%d can't use BT656 and other intf at the same time!\n", dev);
            return -1;
        }
        
        if (type & VO_INTF_CVBS)
        {
            printf("Hd%d can't use CVBS!\n", dev);
            return -1;
        }
        
        if (type & ~(VO_INTF_VGA | VO_INTF_YPBPR | VO_INTF_BT1120
            | VO_INTF_HDMI | VO_INTF_BT656))
        {
            printf("Hd%d unknow intf%d!\n", dev, type);
            return -1;
        }        
    }
    else if ((VOU_DEV_DSD0 <= dev) && (VOU_DEV_DSD5 >= dev))
    {
        if ((VOU_DEV_DSD0 == dev) || (VOU_DEV_DSD1 == dev))
        {
            if (type & (~(VO_INTF_BT656 | VO_INTF_CVBS)))
            {
                printf("Sd%d can't use intf except CVBS and BT656!\n", dev);
                return -1;
            }
        }
        else
        {
            if (type & (~VO_INTF_BT656))
            {
                printf("Sd%d can't use intf except BT656!\n", dev);
                return -1;
            }
        }
    }
    ...
    if ((VO_INTF_VGA & type) || (VO_INTF_BT1120 & type) || (VO_INTF_HDMI & type))
    {
    ...
    if (VO_INTF_YPBPR & type)
    {
        if (sync < VO_OUTPUT_720P50 || sync > VO_OUTPUT_1080P60)
        {
            printf("Vo%d's intfsync illegal!\n", dev);
            return -1;
        }
    ...
    if (VO_INTF_CVBS & type || VO_INTF_BT656 & type)
    {
...
int do_startvo(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (dev >= VOU_DEV_DSD2 || sync >= VO_OUTPUT_BUTT)
    {
        printf("Invalid parameter!\n");
        return -1;
    }
...
U_BOOT_CMD(
	...
	"\nargs: [dev, type, sync]\n"
	"\t-<dev> : 0~1(HD0~1), 2~3(SD0~1)\n"
	"\t-<type>: 1(CVBS),2(YPbPr),4(VGA),8(BT.656),16(BT.1120),32(HDMI)\n"
