...
int check_vo_support(unsigned int dev, unsigned int type, unsigned int sync)
{
    ...
    {
        if((VO_INTF_VGA != type) && (VO_INTF_HDMI != type) && ((VO_INTF_VGA | VO_INTF_HDMI) != type))
        {
            printf("Hd%d only supports HDMI and VGA intftype, intf %d is illegal!\n",dev,type);
            return -1;
    ...
    /*检查时序*/
    if((VO_INTF_VGA & type)||(VO_INTF_HDMI & type))
    {
