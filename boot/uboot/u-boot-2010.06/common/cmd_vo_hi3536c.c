...
int check_vo_support(unsigned int dev, unsigned int type, unsigned int sync)
{
	...
	else if (VOU_DEV_DSD0 == dev)
	{
		if(VO_INTF_CVBS != type)
		{
			printf("Sd%d only supports CVBS intftype, intf %d is illegal!\n",dev,type);
			return -1;
		}
	}
	else
	...
	{
		if (sync < VO_OUTPUT_1080P24 || sync > VO_OUTPUT_3840x2160_30)
		{
			printf("Vo%d's intfsync %d illegal!\n", dev, sync);
			return -1;
		}
...
int do_startvo(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (dev > VOU_DEV_DSD0 || sync >= VO_OUTPUT_BUTT)
    {
        printf("Invalid parameter!\n");
        return -1;
    }
...
int do_startgx(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (layer >= VO_GRAPHC_G4 ||layer < VO_GRAPHC_G0 || strd > (PIC_MAX_WIDTH*2)
...
int do_startvl(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (layer > VOU_LAYER_VSD0 || layer == VOU_LAYER_VP || layer < VOU_LAYER_VHD0
