...
static int hinfc504_hynix_cg_adie_set_rr_reg(struct hinfc_host *host, char *val)
{
	...
	for (i = 0; i <= 8; i++) {
		...
		case 0:
			...
			hinfc_write(host, hinfc504_hynix_cg_adie__rr_reg[i],
				HINFC504_ADDRL);
			hinfc_write(host,
				0x36, HINFC504_CMD);
			hinfc_write(host,
...
static int hinfc504_hynix_cg_adie_set_rr_param(struct hinfc_host *host,
	int param)
{
	unsigned char *rr;
...
static int hinfc504_hynix_cg_adie_eslc_enable(struct hinfc_host *host,
	int enable)
{
	int ix;
