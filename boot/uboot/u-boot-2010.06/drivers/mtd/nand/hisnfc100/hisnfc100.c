...
unsigned int nand_ip_version = HISNFC_VER_100;
static struct hisnfc_host snfc_host = {
	.chip = NULL,
};
...
{
	int ix;
	unsigned int regbase = CONFIG_HISNFC100_REG_BASE_ADDRESS;
...
static void hisnfc100_send_cmd_pageprog(struct hisnfc_host *host)
{
	...
	if (DEBUG_WRITE) {
		...
		if (val & STATUS_P_FAIL_MASK)
			printf("hisnfc100: %s page program failed!" \
				" status[%#x]\n", op_type, val);
	}

	if (DEBUG_WRITE)
		printf("* End %s page program!\n", op_type);
}
...
static void hisnfc100_send_cmd_readstart(struct hisnfc_host *host)
{
	...
	if (DEBUG_READ)
		printf("* Enter %s page read start!\n", op_type);

	if ((host->addr_value[0] == host->cache_addr_value[0])
		&& (host->addr_value[1] == host->cache_addr_value[1])) {
		if (DEBUG_READ)
			printf("* %s page read cache hit! addr1[%#x], " \
...
static void hisnfc100_send_cmd_erase(struct hisnfc_host *host)
{
	...
	val = spi->driver->wait_ready(spi);
	if (val) {
		printf("hisnfc: erase wait ready fail! status[%#x]\n", val);
		return;
	}
	...
	if (DEBUG_ERASE) {
		...
		if (val & STATUS_E_FAIL_MASK)
			printf("hisnfc100: erase failed! status[%#x]\n", val);
	}

	if (DEBUG_ERASE)
		printf("* End send cmd erase!\n");
}
...
static void hisnfc100_send_cmd_status(struct hisnfc_host *host)
{
	...
	if (DEBUG_ERASE || DEBUG_WRITE)
		printf("hisnfc100: %s get %#x status[%#x]\n",
...
static int hisnfc100_dev_ready(struct mtd_info *mtd)
{
	...
	printf("Wait spi nand flash ready timeout.\n");
...
{
	memset((char *)chip->IO_ADDR_R, 0xff, HISNFC100_BUFFER_LEN);
...
int hisnfc100_host_init(struct hisnfc_host *host)
{
	...
	host->iobase  = (void __iomem *)CONFIG_HISNFC100_BUFFER_BASE_ADDRESS;
	host->regbase = (void __iomem *)CONFIG_HISNFC100_REG_BASE_ADDRESS;
	...
	if (((regval & DEVICE_TYPE_MASK) >> DEVICE_TYPE_SHIFT)
		...
		printf("%s: Device type(SPI nor flash) error.\n", __func__);
		return -ENXIO;
	}
	...
	host->buforg = kmalloc((HISNFC100_BUFFER_LEN + HISNFC100_DMA_ALIGN),
				GFP_KERNEL);
	if (!host->buforg) {
		printf("hisnfc100: Can't malloc memory for NAND driver.\n");
		return 1;
	}

	/* 32 bytes alignment */
	host->buffer = (char *)((unsigned int)(host->buforg
		+ HISNFC100_DMA_ALIGN - 1) & ~(HISNFC100_DMA_ALIGN - 1));
...
/*****************************************************************************/
static int hisnfc100_version_check(void)
{
	unsigned regval;
	regval = readl(CONFIG_HISNFC100_REG_BASE_ADDRESS + HISNFC100_VERSION);

	return (HISNFC_VER_100 != regval);
}

/*****************************************************************************/
int board_nand_init(struct nand_chip *nand)
{
	struct hisnfc_host *host = &snfc_host;

	if (host->chip)
		return 0;

	printf("Check SPI Nand Flash Controller v100 ... ");
	if (hisnfc100_version_check()) {
		printf("\n");
		return -ENODEV;
	}
	printf("Found\n");

	memset((char *)host, 0, sizeof(struct hisnfc_host));

	host->chip = nand;
	if (hisnfc100_host_init(host)) {
		printf("hisnfc100: failed to allocate device buffer.\n");
		host->chip = 0;
		return -ENOMEM;
	}
...
int nand_get_ecctype(void)
{
	...
	return spi_nand_ecc_type2reg(snfc_host.ecctype);
}
