			...
			u32 ep_index, trb_type cmd)
{
	...
	XHCI_BUG_ON(prepare_ring(ctrl, ctrl->cmd_ring, EP_STATE_RUNNING));
		...
		int timer)
{
	...
	do {
		...
		/* TODO: remove this once enumeration has been reworked */
			 ...
			 */
			XHCI_BUG_ON(GET_COMP_CODE(
				le32_to_cpu(event->generic.field[2])) !=
								COMP_SUCCESS);
		else
			debug("Unexpected XHCI event TRB, skipping... " \
				"(%08x %08x %08x %08x)\n",
	...
	} while (get_timer(ts) < timer);

	if (expected == TRB_TRANSFER)
		return NULL;

	debug("XHCI timeout on event type %d... cannot recover.\n", expected);
	XHCI_BUG();
	return NULL;
}
 ...
 */
static void abort_td(struct usb_device *udev, int ep_index, int timer)
{
	struct xhci_ctrl *ctrl = udev->controller;
	...
	event = xhci_wait_for_event(ctrl, TRB_TRANSFER, timer);
	field = le32_to_cpu(event->trans_event.flags);
	XHCI_BUG_ON(TRB_TO_SLOT_ID(field) != udev->slot_id);
	XHCI_BUG_ON(TRB_TO_EP_INDEX(field) != ep_index);
	XHCI_BUG_ON(GET_COMP_CODE(le32_to_cpu(event->trans_event.transfer_len
		!= COMP_STOP)));
	xhci_acknowledge_event(ctrl);

	event = xhci_wait_for_event(ctrl, TRB_COMPLETION, timer);
	XHCI_BUG_ON(TRB_TO_SLOT_ID(le32_to_cpu(event->event_cmd.flags))
				   ...
				   union xhci_trb *event, int length)
{
	...
	case COMP_SUCCESS:
		XHCI_BUG_ON(udev->act_len != length);
		/* fallthrough */
	case COMP_SHORT_TX:
		udev->status = 0;
		break;
			...
			int length, void *buffer)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_TRANSFER, XHCI_CTRL_TIMEOUT);
	if (!event) {
		debug("XHCI bulk transfer timed out, aborting...\n");
		abort_td(udev, ep_index, XHCI_ABORT_TIMEOUT);
	...
	XHCI_BUG_ON(TRB_TO_SLOT_ID(field) != slot_id);
	XHCI_BUG_ON(TRB_TO_EP_INDEX(field) != ep_index);
	XHCI_BUG_ON(*(void **)(uintptr_t)le64_to_cpu(event->trans_event.buffer)
