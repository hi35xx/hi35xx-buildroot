/*
 * URB OHCI HCD (Host Controller Driver) for USB on the Hisilicon.
 *
 * (C) Copyright 2003
 ...
 */
static char ohci_inited;

#ifdef CONFIG_HI3518
#include "hiusb-3518.c"
#endif
#ifdef CONFIG_GODNET
#include "hiusb-godnet.c"
#endif
#ifdef CONFIG_GODARM
#include "hiusb-godarm.c"
#endif
...
int usb_lowlevel_init(void)
{
	...
	gohci.sleeping = 0;
	gohci.irq = -1;
	gohci.regs = (struct ohci_regs *)HIUSB_OHCI_BASE;

	gohci.flags = 0;
	gohci.slot_name = HIUSB_OHCI_DEV_NAME;

	if (hc_reset(&gohci) < 0) {
		hc_release_ohci(&gohci);
		/* Initialization failed */
		hiusb_ohci_disable_clk();
		return -1;
	}
