...
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION, XHCI_CTRL_TIMEOUT);
...
{
	union xhci_trb *event;
	struct xhci_ctrl *ctrl = xhcic;
	int ret;

	/*tell the udev current controller */
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION, XHCI_CTRL_TIMEOUT);
	XHCI_BUG_ON(GET_COMP_CODE(le32_to_cpu(event->event_cmd.status))
		!= COMP_SUCCESS);
 ...
 */
int
submit_int_msg_ex(struct usb_device *udev, unsigned long pipe, void *buffer,
 ...
 */
int
submit_bulk_msg_ex(struct usb_device *udev, unsigned long pipe, void *buffer,
								int length)
{
 ...
 */
int
submit_control_msg_ex(struct usb_device *udev, unsigned long pipe, void *buffer,
