...
{
	XHCI_BUG_ON((void *)addr == NULL || len == 0);
		...
		*xhci_alloc_container_ctx(struct xhci_ctrl *ctrl, int type)
{
	...
	XHCI_BUG_ON((type != XHCI_CTX_TYPE_DEVICE)
			&& (type != XHCI_CTX_TYPE_INPUT));
	ctx->type = type;
...
{
	XHCI_BUG_ON(ctx->type != XHCI_CTX_TYPE_INPUT);
