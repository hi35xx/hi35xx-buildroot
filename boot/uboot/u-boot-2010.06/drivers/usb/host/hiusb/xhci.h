...
#define XHCI_BUG_ON(condition) do { if (unlikely((condition) != 0)) \
	XHCI_BUG(); \
} while (0)
...
typedef enum {
	...
	COMP_STALL, /* 6 */
	/* Resource Error - HC doesn't have memory for device configuration */
	COMP_ENOMEM, /* 7 */
	...
	/* Isoc Buffer Overrun an isoc IN ep sent more data than
	 *could fit in TD */
	COMP_BUFF_OVER = 31,
	...
	COMP_STRID_ERR, /* 34 */
	/* Secondary Bandwidth Error - may be returned by a
	 * Configure Endpoint cmd */
	COMP_2ND_BW_ERR, /* 35 */
