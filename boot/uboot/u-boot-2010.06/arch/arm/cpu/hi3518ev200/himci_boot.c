...
#include "himci_boot_hi3518ev200.c"
#endif
...
static void emmc_update_clk(void)
{
	...
	tmp_reg = (START_CMD | USE_HOLD_REG | UP_CLK_ONLY | CARD_NUM);
...
static void emmc_send_cmd(unsigned cmd, unsigned arg, unsigned wait_busy)
{
	...
	himci_writel(cmd | USE_HOLD_REG | WT_PD_CPT | START_CMD | CARD_NUM,
			mmc_base + MCI_CMD);
...
static unsigned int emmc_init(void)
{
	...
	/* set data width */
	himci_writel(CARD_WIDTH_4BIT << 1, mmc_base + MCI_CTYPE);
	...
	himci_writel(0x1 << 1, mmc_base + MCI_CLKENA);
	emmc_update_clk();
	...
	/* set data width */
	himci_writel(CARD_WIDTH_8BIT << 1, mmc_base + MCI_CTYPE);
