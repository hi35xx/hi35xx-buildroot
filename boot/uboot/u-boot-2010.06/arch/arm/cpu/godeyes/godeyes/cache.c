...
void L2CleanAuto(unsigned int wayNum)
{
    ...
    reg = (wayNum << BIT_L2_MAINT_AUTO_WAYADDRESS) | (0x1 << BIT_L2_MAINT_AUTO_START) | (0x1 << BIT_L2_MAINT_AUTO_CLEAN);
        
    L2_REG_WRITE(reg, REG_L2_CACHE_MAINT_AUTO);
    
    while(!( L2_REG_READ(REG_L2_CACHE_RAW_INT) & ( 0X1 << BIT_L2_RINT_AUTO_END ))); 
...
*	
*******************************************************************************/ 
void L2InvalidAuto(unsigned int wayNum)      /* need invalid cache way num */
{
    unsigned int reg;
    
    reg = L2_REG_READ(REG_L2_CACHE_SYNC);  
    
    reg = (wayNum << BIT_L2_MAINT_AUTO_WAYADDRESS) | (0x1 << BIT_L2_MAINT_AUTO_START);
