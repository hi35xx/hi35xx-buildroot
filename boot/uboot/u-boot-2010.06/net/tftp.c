...
TftpSend (void)
{
	...
	case STATE_ACK:
		...
		*s++ = htons(TftpBlock+1);
		...
		uplen = (upload_size - TftpBlockWrapOffset - TftpBlock*TftpBlkSize);
		uplen = uplen > TftpBlkSize ? TftpBlkSize : uplen;
		memcpy((void*)pkt, (const char*)upload_addr + TftpBlockWrapOffset + TftpBlock*TftpBlkSize , uplen);
		pkt += uplen;
		len = pkt - xp;
		break;
	default:
