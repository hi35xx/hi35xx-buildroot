...
#define DUMP_AIAO_HAL_REG(u32Offset)\
    printf("addr: %8x  value: %x\n",((HI_U32)u32AioBase + (HI_U32)u32Offset), (AIAO_HAL_READ_REG(u32Offset)))

#define DUMP_AIAO_HAL_CRG_REG()\
    printf("addr: %8x  value: %x\n",(), (AIAO_HAL_READ_CRG_REG(u32RegAddr)))
...
HI_S32 AIAO_HAL_SysInit(HI_VOID)
{
    ...
    /* hi3535 acodec有杂音,映射acodec的寄存器,以便对acodec进行操作 */
    g_acodec_fast_reg = (void *)IO_ADDRESS(AUDIO_CODEC_REG_ADDR);
    g_acodec_ctrl0_reg = (void *)IO_ADDRESS(AUDIO_CODEC_CTL0_REG_ADDR);
    if (NULL == g_acodec_fast_reg || NULL == g_acodec_ctrl0_reg) 
    {
...
HI_S32 SYS_HAL_AioClkSel(HI_VOID)
{
    ...
    acodec_Ctr0setbit(u32Tmp, 3, IO_ADDRESS(CRG_PERCTL32_ADDR));
    acodec_Ctr0setbit(u32Tmp, 2, IO_ADDRESS(CRG_PERCTL32_ADDR));
...
HI_S32 SYS_HAL_AioClkEn(HI_BOOL bClkEn)
{
    ...
    acodec_Ctr0setbit(u32Tmp, 1, IO_ADDRESS(CRG_PERCTL32_ADDR));
...
HI_S32 SYS_HAL_AioResetSel(HI_BOOL bReset)
{
    ...
    acodec_Ctr0setbit(u32Tmp, 0, IO_ADDRESS(CRG_PERCTL32_ADDR));
