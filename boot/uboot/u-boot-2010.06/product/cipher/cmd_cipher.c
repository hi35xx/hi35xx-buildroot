...
HI_S32 RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_E enScheme)
{
    ...
    printf("RSA Sign Use Clock Cycle: %u, Clock Cycle for 1S: %u\n", time_start, CONFIG_SYS_HZ);
    ...
    time_start = get_timer(time_start);
    printf("RSA Verify Use Clock Cycle: %u, Clock Cycle for 1S: %u\n", time_start, CONFIG_SYS_HZ);
...
HI_S32 PKCS_PRI_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_E enScheme)
{
    ...
    ret = HI_UNF_CIPHER_KladEncryptKey(HI_UNF_CIPHER_KEY_SRC_EFUSE_1, HI_UNF_CIPHER_KLAD_TARGET_RSA, 
            u8Key, stRsaEnc.stPriKey.pu8D, stRsaEnc.stPriKey.u16DLen);
    if(HI_SUCCESS != ret)
	{
	    HI_ERR_CIPHER("Error: Klad Encrypt Key failed!\n");
		return HI_FAILURE;	
	}
    Cipher_printBuffer("ENC_D", stRsaEnc.stPriKey.pu8D, stRsaEnc.stPriKey.u16DLen);
...
static HI_U8 aes_128_enc_key_zero[16]  = {0xec,0x8c,0xa4,0x6e,0x7c,0xd9,0x09,0x74,0x85,0xaf,0x67,0xff,0xbc,0x91,0xa0,0x23};
...
HI_S32 klad_test()
{
    ...
    Cipher_printBuffer("encrypt key", u8Key, 16);
    s32Ret  = memcmp(u8Key, aes_128_enc_key, 16) ;
    s32Ret &= memcmp(u8Key, aes_128_enc_key_zero, 16) ;
    if ( s32Ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Memcmp encrypt key failed!\n");
        return HI_FAILURE;	
    }
    ...
    memset(u8InBuf, 0x0, u32TestDataLen);
    memcpy(u8InBuf, aes_128_src_buf, u32TestDataLen);
    Cipher_printBuffer("clear text:", u8InBuf, u32TestDataLen);

    memset(u8OutBuf, 0x0, u32TestDataLen);

    s32Ret = HI_UNF_CIPHER_Encrypt(hTestchnid, (HI_U32)u8InBuf, (HI_U32)u8OutBuf, u32TestDataLen);
    if(HI_SUCCESS != s32Ret)
	{
    ...
    Cipher_printBuffer("encrypted text:", u8OutBuf, u32TestDataLen);

    if ( 0 != memcmp(u8OutBuf, aes_128_dst_buf, u32TestDataLen) )
    {
   ...
   /* For decrypt */
    memcpy(u8InBuf, aes_128_dst_buf, u32TestDataLen);
    memset(u8OutBuf, 0x0, u32TestDataLen);
    ...
    Cipher_printBuffer("before decrypt:", u8InBuf, u32TestDataLen);
    s32Ret = HI_UNF_CIPHER_Decrypt(hTestchnid, (HI_U32)u8InBuf, (HI_U32)u8OutBuf, u32TestDataLen);
    if(HI_SUCCESS != s32Ret)
	{
		HI_ERR_CIPHER("Cipher decrypt failed.\n");
		s32Ret = HI_FAILURE;
    ...
    Cipher_printBuffer("decrypted text:", u8OutBuf, u32TestDataLen);
	/* compare */
    if ( 0 != memcmp(u8OutBuf, aes_128_src_buf, u32TestDataLen) )
    {
        HI_ERR_CIPHER("Memcmp failed!\n");
        s32Ret = HI_FAILURE;
        goto __CIPHER_EXIT__;
...
U_BOOT_CMD(klad_test, 1, 1, klad_test, "KLAD Test", "for example: klad_test");
