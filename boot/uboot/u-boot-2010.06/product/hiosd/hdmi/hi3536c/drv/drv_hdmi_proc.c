...
/* when HDMI was not open, do not print any info when cat HDMI proc. */
#define CHECK_RET_SUCCESS(ret) \
do{ \
	if(HI_SUCCESS != ret){ \
		HDMI_WARN("Error In %s,Line %d,return not success!\n",__FUNCTION__,__LINE__); \
		return;\
	} \
}while(0)
#else
...
const static HI_CHAR *s_eActAspectRatio2Str[] = {
    ...
    "UNKNOWN"            //HDMI_ACTIVE_ASPECT_BUTT
};
...
const static HI_CHAR *s_ePicAspectRatio2Str[] = {
	...
	"UNKNOWN",		//HDMI_PICTURE_ASPECT_BUTT,
 };
...
const static HI_CHAR *s_eYCCQuantization2Str[] = {
	...
	"UNKNOWN",			//0b10   HDMI_YCC_QUANTIZATION_RANGE_BUTT
    "UNKNOWN",           //0b11   HDMI_YCC_QUANTIZATION_RANGE_BUTT
};
...
const static HI_CHAR *s_eHdrUserMode2Str[] = {
	...
	"UNKNOWN",		//HDMI_HDR_USERMODE_BUTT,
};
...
const static HI_CHAR *s_eRgbQuantization2Str[] = {
	...
	"UNKNOWN", 			    //HDMI_QUANTIZATION_RANGE_BUTT	    0b11
};
...
const static HI_CHAR *s_eExtColorimetry2Str[] = {
	...
	"UNKNOWN"				//	0b111 HDMI_EXTENDED_COLORIMETRY_BUTT
};
...
const static HI_CHAR *s_SoundIntf2Str[] = {
    ...
    "UNKNOWN",			//HDMI_AUDIO_INTERFACE__BUTT
};
...
const static HI_CHAR *s_aStr_Speaker[] = {
	...
	"UNKNOWN"			//HDMI_EDID_AUDIO_SPEAKER_BUTT    ,
};
...
const static HI_CHAR *s_EstabTiming2Str[] = {
    ...
    "UNKNOWN"	       		//HDMI_EDID_ESTABTIMG_BUTT					,
 };
...
const static HI_CHAR *s_eDefaultAction2Str[] = {
	...
	"UNKNOWN"		//HDMI_DEFAULT_ACTION_BUTT,
};
...
const static HI_CHAR *s_eDispFMT2Str[] = {
...
#if (defined(HDMI_CHIP_HI3536) || defined(HDMI_CHIP_HI3536C))
       ...
       "2560x1440@60",    //HI_HDMI_VIDEO_FMT_2560x1440_60,
       ...
       "FMT_UNKNOWN",         //HI_HDMI_VIDEO_FMT_BUTT
#else
...
const static HI_CHAR *s_eBitDepth2Str[] = {
    ...
    "UNKNOWN"      //HDMI_VIDEO_BITDEPTH_BUTT
};
...
const static HI_CHAR *s_eAudioChannel2Str[] = {
    ...
    "UNKNOWN" //HDMI_AUDIO_FORMAT_BUTT
};
...
static HI_VOID Proc_AppAttrInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCPEnable", pstAppAttr->bHDCPEnable? STR_YES: STR_NO);
...
static HI_VOID Proc_SWStatus(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    PROC_PRINT(pFile, "%-15s: %-20d \n", "PreformatOutTime", pstHdmiDev->stIntfStatus.u32PreformatOutTime);

    /* line 10 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "SetformatInTime", pstHdmiDev->stIntfStatus.u32SetformatInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "SetformatOutTime", pstHdmiDev->stIntfStatus.u32SetformatOutTime);
...
static HI_VOID Proc_EventInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	...
	s32Ret = DRV_HDMI_EventPoolTotalGet(enHdmiID,&u32TotalPool);
	CHECK_RET_SUCCESS(s32Ret);

	for (i = 0; i < u32TotalPool; i++)
	{
		if(DRV_HDMI_EventPoolStatusGet(enHdmiID,i+1,&stCtrl,&stRunCnt)==HI_SUCCESS)
		{
...
static HI_S32 Proc_GetCapInfo(HDMI_DEVICE_ID_E enHdmiID, HDMI_SINK_CAPABILITY_S **pstCap)
{
    ...
    if (HDMI_EDID_DATA_INVALID == s32Ret)
    {
        ...
        if (HDMI_EDID_DATA_INVALID == s32Ret)
        {
            HDMI_WARN("Get the edid capability error, s32Ret=%d, pstCap=%p!\n", s32Ret, pstCap);
            return HI_FAILURE;
        }
    }
...
{
    HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
	pstHdmiDev = GetHdmiDevice(HDMI_DEVICE_ID0);
	CHECK_POINTER_RET(pstHdmiDev);
...
HI_S32 DRV_HDMI_SinkReadProc(struct seq_file *pFile, HI_VOID *pArg)
{
    ...
    {
        HDMI_WARN("call Proc_GetCapInfo failed! s32Ret: %d, pstCap: %p\n", s32Ret, pstCap);
    }
    HDMI_MUTEX_UNLOCK(pstHdmiDev->stMutexProc);
