...
HI_VOID HAL_HDMI_DebugCecPrint(HI_VOID *pvData)
{
    ...
    for (i=0;(i<stStatus.u32TxMsgCnt) && (i<CEC_MAX_MSG_NUM);i++)
    {
        ...
        HDMI_HAL_INFO("\n------------------------- cec %s msg NUM[%03u] ---------------------------\n",pstCecData->bCecTx ? "tx" : "rx",i);
        HDMI_HAL_INFO("%-18s:%-20s|",  "bCecEnable",   STR_BOOL(stElem.bCecEnable));
        HDMI_HAL_INFO("%-18s:%-20s\n",  "enProcess",   STR_PROCESS(stElem.enProcess));

        HDMI_HAL_INFO("%-18s:0x%02x %-15s|","enDestAddr",   stElem.stMsg.enDestAddr,STR_CEC_ADDR(stElem.stMsg.enDestAddr));
        HDMI_HAL_INFO("%-18s:%-20llu\n","u64TimeStamp",   stElem.u64TimeStamp);


        HDMI_HAL_INFO("%-18s:0x%02x %-15s|","enSrcAddr",      stElem.stMsg.enSrcAddr,STR_CEC_ADDR(stElem.stMsg.enSrcAddr));
        HDMI_HAL_INFO("%-18s:0x%02x %-15s\n",  "u8Opcode",    stElem.stMsg.u8Opcode,DebugCecCmdStrGet(stElem.stMsg.u8Opcode));
