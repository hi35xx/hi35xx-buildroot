								...
								HI_U8 u8Len)
{
    ...
    {	
        ...
        if (u32VicLower <= HDMI_3840X2160P60_4_3)
        {
								...
								HI_U8 u8Len )
{
    ...
    if((u8Hdmi3DLen > 0) && (u8Len >= (u8Hdmi3DLen + u8Offset)) && b3DPresent) 
    {
        ...
        if(u83DMultiPresent == EDID_3DMULTI_PRESENT_UPPER8)
        {
            ...
            u8Byte = pau8Data[u8Offset++] & 0xFF;
            //EDID_INFO("    VSDB 3D_MASK_15бн8 (u8Byte-%d):0x%x\n",u8Offset-1,u8Byte);
            u8Byte = pau8Data[u8Offset++] & 0xFF;
            //EDID_INFO("    VSDB 3D_MASK_7бн0 (u8Byte-%d):0x%x\n",u8Offset-1,u8Byte);
        }
    ...
    for(i = u8Offset ; i < u8Len; i++)
    {
        ...
        //EDID_INFO("VSDB 2D_VIC_order:0x%02x\n", u83DStructure);
       // EDID_INFO("    %s%-23d : %02d\n", "2D_VIC_order_", u83DStructure);
        u83DStructure = u8Byte & EDID_LOWER_NIBBLE_MASK;
        if (u83DStructure <= 0x08)
        {
		   // EDID_INFO("    %s: %s\n", "3D_Structure_", u83dModeStr[u83DStructure]);
        }
        
        if(u83DStructure >= 0x08)
        {
            //EDID_INFO("VSDB 3D_Detailed > 0x08 :0x%02x\n", u83DStructure);
        }

        u8Offset++;
       // EDID_INFO("VSDB 3D_Detailed :0x%02x\n", u83DStructure);
    }
										...
										HI_U8 u8Len)
{
	...
	for(i = 0; i < u8Len-1; i++)
    {
		...
		if (u32Vic <= HDMI_3840X2160P60_4_3)
		{
									 				...
									 				HDMI_SINK_CAPABILITY_S **pstCapDest)
{
	...
	*pstCapDest = &pstEdidInfo->stCapability;
	...
	else
	{
        ...
        //DRV_HDMI_EdidUpdate(pstEdidInfo, HDMI_EDID_UPDATE_SINK);
        if(pstStatus->bCapValid)
        {
            return HDMI_EDID_DATA_VALIDSINK;
        }
