...
#define CRG_PERCTL61_ADDR (0x00F4 + CRG_REGS_ADDR)

#define CRG_PERCTL13_ADDR (0x00a4 + CRG_REGS_ADDR)
#define CRG_PERCTL15_ADDR (0x00a8 + CRG_REGS_ADDR)
...
HI_S32 SYS_HAL_DDRConifg(HI_VOID)
{
	...
	//#########################	
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x1212007c),  0x55553301);
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x121140ac),  0x00000040);
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x12120080),  0x33355633);
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x12120084),  0x66266623);
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x12120088),  0x66662666);	


	HAL_WriteReg((HI_U32*)IO_ADDRESS( 0x12120094), 0x65);   //###?2:0?VI   ?6:4?VO
	HAL_WriteReg((HI_U32*)IO_ADDRESS( 0x12110020), 0x00000001);  //# AXI_ACTION[19:8]:wr_rcv_mode=0,12ports 
...
//##############  qosbuf #############
    ...
    HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x121140ec),  0x00000044);   //#row-hit
...
//############## WR timeout ###########
    HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x1211409c),  0x00000a00);  //# wr_tout3 ~wr_tout0
...
//############## RD timeout ###########
	...
	HAL_WriteReg((HI_U32*)IO_ADDRESS(  0x121141f8),  0x00800002);  //# qosb_rhit_ctrl,open_window=128,close_window=2	
...
HI_S32 SYS_HAL_VouHdPllSel(HI_S32 s32VoDev, HI_U32 u32ClkSel)
{
	if (0 == s32VoDev)
	{
		HI_RegWrite32(u32ClkSel<<1, 0x3<<1, IO_ADDRESS(CRG_PERCTL13_ADDR));
	}
	else if (1 == s32VoDev)
	{
		HI_RegSetBit(u32ClkSel, 3, IO_ADDRESS(CRG_PERCTL13_ADDR));
	}
	else
	{}
...
{
    HI_RegSetBit(u32ClkSel, 30, IO_ADDRESS(CRG_PERCTL13_ADDR));
    return 0;
}
...
HI_S32 SYS_HAL_VouPLLClkDiv(HI_S32 s32Pll, HI_U32 u32BitsSet)
{
    if (0 == s32Pll)
    {
        HI_RegWrite32(u32BitsSet << 0, 0x1f << 0, IO_ADDRESS(CRG_PERCTL61_ADDR));
    }
    else if(1 == s32Pll)
    {
        HI_RegWrite32(u32BitsSet << 5, 0x1f << 5, IO_ADDRESS(CRG_PERCTL61_ADDR));
    }
    else
    {
        return -1;
...
HI_VOID HAL_SYS_SetAxiMaster(HI_VOID)
{
    ...
    VOAXISEL.u32 = pVoReg->VOAXISEL.u32;
    VOAXISEL.bits.v0_axi_sel = 0;
    VOAXISEL.bits.v1_axi_sel = 0;
    VOAXISEL.bits.v3_axi_sel = 0;
    VOAXISEL.bits.v4_axi_sel = 0;
    VOAXISEL.bits.g0_axi_sel = 0;
    VOAXISEL.bits.g1_axi_sel = 0;
    VOAXISEL.bits.g2_axi_sel = 0;
    VOAXISEL.bits.g3_axi_sel = 0;
    VOAXISEL.bits.g4_axi_sel = 0;
    VOAXISEL.bits.wbc_dhd_axi_sel = 0;
    VOAXISEL.bits.wbc_g0_axi_sel = 0;
    VOAXISEL.bits.wbc_g4_axi_sel = 0;
    VOAXISEL.bits.para_axi_sel = 0;
	pVoReg->VOAXISEL.u32 = VOAXISEL.u32;
...
{	
    ...
    switch(enLayer)
    {
        ...
        {
            RegAbsAddr = (pReg) + 0 * VHD_REG_LEN;//从VHD0开始映射  V0     
            break;
        }
        case HAL_DISP_LAYER_VHD1:
        {
            RegAbsAddr = (pReg) + 3 * VHD_REG_LEN;//从VHD0开始映射  V3
            break;
        }
        case HAL_DISP_LAYER_VP:
        {
            RegAbsAddr = (pReg) + 1 * VHD_REG_LEN;//从VHD0开始映射  v1
            break;
        }        
        case HAL_DISP_LAYER_VSD0:
        {
            RegAbsAddr = (pReg) + 4 * VHD_REG_LEN;//从VHD0开始映射  v4
            break;
        }
