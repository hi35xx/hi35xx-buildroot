...
{
    {
        .bEnable        = HI_FALSE,
        .u32BkGrd       = VOU_BACKGROUD_DEFAULT,
        .enIntfType     = VO_INTF_YPBPR | VO_INTF_HDMI,
...
{        
    ...
    for (i=HAL_DISP_LAYER_GFX0; i<=HAL_DISP_LAYER_HC1; i++)
    {
        HAL_GRAPHIC_SetGfxPalphaRange(i, 1);
    }
 
    for (i=0; i<=HAL_DISP_LAYER_VSD4; i++)
    {
        HAL_VIDEO_SetIfirMode(i, HAL_IFIRMODE_6TAPFIR);
    }
    HAL_VIDEO_SetIfirMode(HAL_DISP_LAYER_VHD2, HAL_IFIRMODE_COPY);
  
    HAL_DISP_SetIntfDfirEn(HAL_DISP_CHANNEL_DHD0,0);
    HAL_DISP_SetIntfDfirEn(HAL_DISP_CHANNEL_DSD0,0);
    HAL_DISP_SetIntfDfirEn(HAL_DISP_CHANNEL_DHD1,0);
...
{
    U_HDATE_VIDEO_FORMAT enVFormat;
    U_HDATE_OUT_CTRL enOutCtrl;
    U_HDATE_POLA_CTRL enPolaCtrl;
    ...
    switch (enOutSync)
    {
        ...
        {
            if(g_stHalDevCfg[VoDev].enIntfType & VO_INTF_VGA)
            {
                 enVFormat.bits.csc_ctrl = 0x2;
                 enVFormat.bits.video_out_ctrl = 0x0;
            }
            else if(g_stHalDevCfg[VoDev].enIntfType & VO_INTF_YPBPR)
            {
                enVFormat.bits.csc_ctrl = 0x0;
                enVFormat.bits.video_out_ctrl = 0x1;
            }
            else
            {
                return;
            }
            enVFormat.bits.video_ft = 0x0;
            break;
        }
    ...
    enVFormat.bits.sync_add_ctrl = 0x2;
    
    enOutCtrl.bits.video1_sel = 0x1;
    enOutCtrl.bits.video2_sel = 0x2;
    enOutCtrl.bits.video2_sel = 0x3;

    enPolaCtrl.bits.hsync_in_pola = 0x1;
    enPolaCtrl.bits.vsync_in_pola = 0x1;

    HAL_DISP_HDATE_VideoFmt(VoDev, &enVFormat);
    HAL_DISP_HDATE_OutCtrl(VoDev, &enOutCtrl);
    HAL_DISP_HDATE_PolaCtrl(VoDev, &enPolaCtrl);

    HAL_DISP_HDATE_Enable(VoDev,HI_TRUE);
...
HI_VOID VOU_DRV_DevIntEnable(HI_S32 VoDev, HI_BOOL Enable)
{
    ...
    switch ( VoDev )
    {
        ...
        case VOU_DEV_DSD1 :
            ...
            break;
        case VOU_DEV_DSD2 :
            IntType = VOU_INTMSK_DSDVTTHD2;
            break;
        case VOU_DEV_DSD3 :
            IntType = VOU_INTMSK_DSDVTTHD3;
            break;
        case VOU_DEV_DSD4 :
            IntType = VOU_INTMSK_DSDVTTHD4;
            break;
        case VOU_DEV_DSD5 :
            IntType = VOU_INTMSK_DSDVTTHD5;
            break;
        default:
            return;
    }
...
{
    HAL_LAYER_SetVouDdrSel(VoLayer, enDdrSelId);
    return;
}
...
HI_S32 VOU_DRV_GraphicsSetCscCoef(VO_LAYER VoLayer)
{
    ...
    HI_S32 s32LayerIndex = VoLayer - HAL_DISP_LAYER_GFX0;
...
HI_VOID VOU_DRV_LayerCSCConfig(VO_LAYER VoLayer, HI_S32 u32Luma, HI_U32 u32Contrast, HI_U32 u32Hue, HI_U32 u32Satuature)
{
    ...
    else if (VoLayer == VOU_LAYER_VHD2)
    {
        enCscMode = HAL_CSC_MODE_BT601_TO_BT709;
    }
    else if ((VoLayer > VOU_LAYER_VHD2) && (VoLayer <= VOU_LAYER_VSD4))
    {
        enCscMode = HAL_CSC_MODE_BT601_TO_BT601;
    }
    else
...
HI_VOID VOU_DRV_DefLayerBindDev(HI_VOID)
{
    ...
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VHD2, VOU_MIX_PRIO1, VOU_DEV_DHD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G0, VOU_MIX_PRIO2, VOU_DEV_DHD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G4, VOU_MIX_PRIO3, VOU_DEV_DHD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_HC0, VOU_MIX_PRIO4, VOU_DEV_DHD0);

    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VHD1, VOU_MIX_PRIO0, VOU_DEV_DHD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VHD2, VOU_MIX_PRIO1, VOU_DEV_DHD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G1, VOU_MIX_PRIO2, VOU_DEV_DHD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G4, VOU_MIX_PRIO3, VOU_DEV_DHD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_HC1, VOU_MIX_PRIO5, VOU_DEV_DHD1);

    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VSD0, VOU_MIX_PRIO0, VOU_DEV_DSD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VSD0, VOU_MIX_PRIO1, VOU_DEV_DSD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G2, VOU_MIX_PRIO2, VOU_DEV_DSD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G4, VOU_MIX_PRIO3, VOU_DEV_DSD0);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_HC1, VOU_MIX_PRIO4, VOU_DEV_DSD0);

    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VSD1, VOU_MIX_PRIO0, VOU_DEV_DSD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_VSD1, VOU_MIX_PRIO1, VOU_DEV_DSD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G3, VOU_MIX_PRIO2, VOU_DEV_DSD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_G4, VOU_MIX_PRIO3, VOU_DEV_DSD1);
    HAL_CBM_SetCbmMixerPrio(VOU_LAYER_HC1, VOU_MIX_PRIO4, VOU_DEV_DSD1);
...
HI_VOID VOU_DRV_SetDevClk(VO_DEV VoDev)
{
    ...
    {
        VO_DEV VoAnotherDev = 1 - VoDev;
        HI_U32 u32Frac, u32Postdiv1, u32Postdiv2, u32Fbdiv, u32Refdiv, u32Value;

        if (g_stHalDevCfg[VoDev].enIntfType & VO_INTF_YPBPR)
        {
            s32Pll = 1;
        }
        else
        {
            if (HI_FALSE == g_stHalDevCfg[VoAnotherDev].bEnable)
            {
                s32Pll = 0;
            }
            else
            {
                if (g_stHalDevCfg[VoAnotherDev].enIntfType & VO_INTF_YPBPR)
                {
                    s32Pll = 0;
                }
                else
...
hd_clk_sel:
        ...
        {
            u32Value = VoDev;
            SYS_HAL_SelVoBt1120Clk(u32Value);
        }
        if (g_stHalDevCfg[VoDev].enIntfType & VO_INTF_VGA)
        {
            u32Value = 1;
            SYS_HAL_SelVoHdDacClk(u32Value);
        }
        if (g_stHalDevCfg[VoDev].enIntfType & VO_INTF_YPBPR)
        {
            u32Value = 0;
            SYS_HAL_SelVoHdDacClk(u32Value);
            u32Value = (1 - VoDev);
            SYS_HAL_SelVoHddateClk(u32Value);
        }
        if (g_stHalDevCfg[VoDev].enIntfType & VO_INTF_HDMI)
        {
            u32Value = VoDev;
            SYS_HAL_SelVoHDMIClk(u32Value);
        }

    #endif

        u32Value = s32Pll;
        SYS_HAL_SelVoHdClk(VoDev, u32Value);
    }
    else
    {
    }
