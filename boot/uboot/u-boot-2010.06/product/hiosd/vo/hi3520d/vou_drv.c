...
HI_VOID VOU_DRV_VGACscConfig(VO_CSC_MATRIX_E enCscMatrix,HI_S32 u32Luma, HI_U32 u32Contrast, HI_U32 u32Hue, HI_U32 u32Satuature)
{
    #if 1
    HAL_CSC_MODE_E enCscMode;    
    HI_BOOL  bIsVgaModeBy709;

    switch (enCscMatrix)
    {
        case VO_CSC_MATRIX_BT601_TO_RGB_PC:            
            bIsVgaModeBy709 = HI_FALSE;
            break;    
        case VO_CSC_MATRIX_BT709_TO_RGB_PC :            
            bIsVgaModeBy709 = HI_TRUE;
            break;
        default :
            enCscMode = VO_CSC_MATRIX_BT709_TO_RGB_PC;
            bIsVgaModeBy709 = HI_TRUE;
            break;
    }    
    HAL_DISP_SetVgaCscMode(bIsVgaModeBy709);
    #endif
    return;
}
...
HI_VOID VOU_DRV_SetDevClk(VO_DEV VoDev)
{
    ...
    if (VOU_DEV_DHD0 == VoDev)
    {
        ...
        switch (g_stHalDevCfg[VoDev].enOutSync)
        {
            ...
            case VO_OUTPUT_NTSC :
            {
                /*20D没有BT1120,BT656，所以对于这个27M的配置无效，这个case在外面就已经受限制了*/
                //27MHz
                u32Fbdiv = 81;
                u32Frac = 0;
                u32Refdiv = 2;
            ...
            {
                #if 0  //消隐区标准版的时序，该时序当前HDMI不可以输出显示
                //193.25MHz
                u32Fbdiv = 773;
                u32Frac = 0;
                u32Refdiv = 16;
                u32Postdiv1 = 3;
                u32Postdiv2 = 1;
                #else  //消隐区缩减版的时序，该时序当前HDMI可以输出显示
                //154MHz
                u32Fbdiv = 154;
                u32Frac = 0;
                u32Refdiv = 4;
...
HI_VOID VOU_DRV_Open(HI_S32 VoDev)
{
    ...
    if (VO_INTF_VGA & g_stHalDevCfg[VoDev].enIntfType)
    {
        VOU_DRV_VGACscConfig(VO_CSC_MATRIX_BT709_TO_RGB_PC,50, 50, 50, 59);
