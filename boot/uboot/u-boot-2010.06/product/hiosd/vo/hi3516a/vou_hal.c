...
HI_S32 SYS_HAL_VouDacPctrl(HI_S32 s32Dev, HI_BOOL pbClkReverse)
{
    ...
    HI_RegSetBit(u32Tmp, 3, IO_ADDRESS(CRG_PERCTL13_ADDR));
...
HI_S32 SYS_HAL_VouCFGClkEn(HI_S32 s32Dev, HI_BOOL pbClkEn)
{
    ...
    if (0 == s32Dev)
    {   
        HI_RegSetBit(u32Tmp, 4, IO_ADDRESS(CRG_PERCTL13_ADDR));
    }
    else
    {
        return -1;
...
HI_S32 SYS_HAL_VouHdClkSel(HI_S32 s32Dev, HI_U32 u32ClkSel)
{
    if (0 == s32Dev)
    {   
        HI_RegWrite32(u32ClkSel << 12, 0x3 << 12, IO_ADDRESS(CRG_PERCTL13_ADDR));
    }
    else
    {
        return -1;
...
{  
    ...
    switch(enIntf)
    {
        case HAL_DISP_INTF_CVBS:
            ...
            VO_DAC_R_CTRL.bits.endac = bDacChnEnable;
...
HI_BOOL  HAL_DISP_SetIntfMuxSel(HAL_DISP_OUTPUTCHANNEL_E enChan,HAL_DISP_INTF_E enIntf)
{
    ...
    switch(enIntf)
    {
        ...
        default:
        {
            printf("Error! DP_DISP_SetIntfMuxSel Wrong Select\n");
            return HI_FALSE;
        }
    }
