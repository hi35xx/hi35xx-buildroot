...
HIMPP_HI3520V100_VERSION = 1.0.3.0
HIMPP_HI3520V100_SITE_METHOD = file
HIMPP_HI3520V100_SITE = $(TOPDIR)/package/himpp/himpp-hi3520v100
HIMPP_HI3520V100_SOURCE = hi3520v100-mpp-$(HIMPP_HI3520V100_VERSION).tgz
HIMPP_HI3520V100_INSTALL_STAGING = YES
HIMPP_HI3520V100_DEPENDENCIES = linux

SENSOR_TYPE = $(call qstrip,$(BR2_PACKAGE_HIMPP_SNS_TYPE))

HIMPP_HI3520V100_MAKE_OPTS = ARCH=arm
HIMPP_HI3520V100_MAKE_OPTS += HIARCH=hi3520A
HIMPP_HI3520V100_MAKE_OPTS += LIBC=uclibc
HIMPP_HI3520V100_MAKE_OPTS += CROSS=$(TARGET_CROSS)
HIMPP_HI3520V100_MAKE_OPTS += CROSS_COMPILE=$(TARGET_CROSS)
HIMPP_HI3520V100_MAKE_OPTS += LINUX_ROOT=$(LINUX_DIR)
HIMPP_HI3520V100_MAKE_OPTS += MPP_PATH=$(@D)
HIMPP_HI3520V100_MAKE_OPTS += SENSOR_TYPE=$(SENSOR_TYPE)

define HIMPP_HI3520V100_EXTRACT_CMDS
	mkdir -p $(@D)
	$(TAR) -zxf $(DL_DIR)/$(HIMPP_HI3520V100_SOURCE) \
		--strip-components=2 -C $(@D)
endef

define himpp_build
	( cd $(@D)/$(1) && $(MAKE1) $(HIMPP_HI3520V100_MAKE_OPTS) ) || exit 1;
endef

define HIMPP_HI3520V100_BUILD_CMDS
	( cd $(@D)/lib_hi3520A; \
	for f in *.a; do \
...
ifeq ($$(BR2_PACKAGE_HIMPP_EXTDRV_$(1)),y)
	HIMPP_HI3520V100_BUILD_CMDS += $$(call himpp_build,extdrv_hi3520A/$(2))
...
ifeq ($$(BR2_PACKAGE_HIMPP_SAMPLES_$(call qstrip,$(1))),y)
	HIMPP_HI3520V100_BUILD_CMDS += $$(call himpp_build,sample/hi3520A/$(2))
...
define HIMPP_HI3520V100_INSTALL_STAGING_CMDS
	mkdir -p $(STAGING_DIR)$(HIMPP_PREFIX)/include/hi3520v100mpp
	mkdir -p $(STAGING_DIR)$(HIMPP_PREFIX)/lib
	# install header files
	cp -a $(@D)/include_hi3520A/* \
	    $(STAGING_DIR)$(HIMPP_PREFIX)/include/hi3520v100mpp/
...
define HIMPP_HI3520V100_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -m 0755 -D package/himpp/himpp-hi3520v100/load3520.sh \
	    $(MPPDRV_TARGET_DIR)/load3520.sh
endef

define HIMPP_HI3520V100_INSTALL_INIT_SYSV
	$(INSTALL) -m 0755 -D package/himpp/himpp-hi3520v100/S25himpp \
