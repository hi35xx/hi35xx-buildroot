...
#if 0
#define dbg(x) do {\
	printk(KERN_INFO "BOOTINFO-CMDLINE-PART: ");\
	printk x; } while (0)
#else
#define dbg(x)
#endif
		...
		int extra_mem_size)
{
	...
	/* enter this partition (offset will be calculated
	   later if it is zero at this point) */
...
static int __init bootpart_cmdline_parse(char *s)
{
	...
	parts = newbootpart(s,		/* cmdline */
			...
			/*alignment*/
			sizeof(*this_bootpart) + sizeof(void *)-1);
	if (!parts) {
		/*
