...
static void restore_file_descriptors(struct file_object *fobj)
{
	...
	list_for_each_entry_safe(fdesc, tmp, &fobj->fdesc_list, same_fobj) {
		...
		if (unlikely(!fobj->file)) {
			pr_err("PM:FS: have no file to restore "
					"[%s(%d)][fd:%d]\n",
	...
	list_for_each_entry_safe(fobj, tmp, &fpath->fobj_list, same_fpath) {
		...
		if (unlikely(IS_ERR(file))) {
			...
			if (IS_ERR(file)) {
				...
				pr_err("PM:FS: got noentry file instead of [%s]\n",
						filename);
			}
		} else {
			/*
			...
			if (unlikely(fobj->pos >
						i_size_read(file->
							f_dentry->d_inode))) {
				pr_err("PM:FS: file size (%lld) "
						"smaller than offset (%lld)\n",
						i_size_read(file->f_dentry->
							d_inode), fobj->pos);
...
static int check_file_busy(struct task_struct *task, struct file *file)
{
	...
	if (unlikely(nr_mappings)) {
		busy = 1;
		pr_err("PM:FS: file [%s] (opened by [%s:%d]) "
				"has been mmapped (%d)\n",
				d_path_of_file(file), &task->comm[0],
				task->pid, nr_mappings);
	}
...
retry:
	...
	ret = idr_get_new_above(&group->inotify_data.idr,
			&tmp_ientry->fsn_entry,
				wd, &tmp_ientry->wd);
...
static void restore_saved_inotify_users(void)
{
	...
	list_for_each_entry_safe(saved, tmp, &saved_inotify_users, node) {
		...
		if (error /* -ENOENT */) {
			...
			pr_err("PM:FS: got noentry file and put inotify user on it\n");
			path = file->f_path;
			path_get(&path);
			/*
...
#else
static inline int save_inotify_users_on_rw_partitions(void) { return 0; }
static inline int restore_saved_inotify_users(void) { return 0; }
