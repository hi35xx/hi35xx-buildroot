...
static size_t copy_page_to_iter_iovec(struct page *page,
					size_t offset,
					size_t bytes,
					struct iov_iter *i)
{
...
static size_t copy_page_from_iter_iovec(struct page *page,
					size_t offset,
					size_t bytes,
					struct iov_iter *i)
{
...
static void memcpy_to_page(struct page *page,
			size_t offset,
			char *from,
			size_t len)
{
...
static size_t copy_to_iter_bvec(void *from,
				size_t bytes,
				struct iov_iter *i)
{
	...
	while (bytes) {
		bvec++;
		copy = min_t(size_t, bytes, (size_t)bvec->bv_len);
...
static size_t zero_bvec(size_t bytes, struct iov_iter *i)
{
	...
	while (bytes) {
		bvec++;
		copy = min_t(size_t, bytes, bvec->bv_len);
