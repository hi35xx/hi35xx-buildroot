				  ...
				  struct dma_desc *p)
{
	 ...
	 * Notice: TOE make bit 1, crc_error, represent payload_csum_error.
	 * bit 0, extend status bit, is set if TOE, reset if bypass. */
	ret = enh_desc_coe_rdes0(p->des01.erx.ipc_csum_error,
		p->des01.erx.frame_type, p->des01.erx.crc_error);
				  ...
				  int disable_rx_ic)
{
	...
	for (i = 0; i < ring_size; i++) {
		p->des01.erx.own = 1;
		p->des01.erx.buffer1_size = DMA_BUFFER_DESC_SIZE;
...
static void enh_desc_set_tx_igmp(struct dma_desc *p)
{
	p->des01.etx.crc_disable = 1;
}
static void enh_desc_clean_tx_igmp(struct dma_desc *p)
{
	p->des01.etx.crc_disable = 0;
}
