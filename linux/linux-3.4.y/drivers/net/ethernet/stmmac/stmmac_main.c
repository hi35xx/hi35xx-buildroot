...
static int rfa = RFA_VALUE;
module_param(rfa, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(rfa,
	"Threshold(Fill level of Rx FIFO) for activating flow control");

static int rfd = RFD_VALUE;
module_param(rfd, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(rfd,
	"Threshold(Fill level of Rx FIFO) for deactivating flow control");
...
#define CONNECTIONS_MIN_VALUE	(3)
...
static char *phy0_mode;
module_param(phy0_mode, charp, 0);
MODULE_PARM_DESC(phy0_mode, "Phy0 interface mode(mii/rmii/rgmii).");

static char *phy1_mode;
module_param(phy1_mode, charp, 0);
MODULE_PARM_DESC(phy1_mode, "Phy1 interface mode(mii/rmii/rgmii).");
...
static void stmmac_verify_args(void)
{
	...
	if (unlikely(rfa < 1 || rfa > 7))
		rfa = RFA_VALUE;
	if (unlikely(rfd < 1 || rfd > 7))
		rfd = RFD_VALUE;
	if (unlikely(rfa > rfd))
		rfd = rfa;
}
				  ...
				  int speed, int full_duplex, int link_up)
{
	...
	/* support RGMII, MII, RGMII mode */
	if (priv->phy_interface == PHY_INTERFACE_MODE_RGMII)
		newval |= 0x20;
	else if (priv->phy_interface == PHY_INTERFACE_MODE_RMII)
		newval |= 0x80;
...
static inline void stmmac_disable_mac_tx(void __iomem *ioaddr)
{
	u32 value = readl(ioaddr + MAC_CTRL_REG);
...
static inline void stmmac_disable_mac_rx(void __iomem *ioaddr)
{
	u32 value = readl(ioaddr + MAC_CTRL_REG);
...
static int init_dma_desc_rings(struct net_device *dev)
{
	...
	if ((priv->rx_skbuff_dma == NULL) || (priv->rx_skbuff == NULL)) {
		pr_err("%s:ERROR kzalloc rx_skbuff_dma/rx_skbuff\n", __func__);
		return -ENOMEM;
	}

	if ((priv->tx_skbuff == NULL) || (priv->tx_page == NULL)) {
		pr_err("%s:ERROR kzalloc the tx_skbuff/tx_page\n", __func__);
		return -ENOMEM;
	}
...
static void dma_free_tx_skbufs(struct stmmac_priv *priv)
{
	...
	for (i = 0; i < priv->dma_tx_size; i++) {
		...
		if (priv->tx_skbuff && priv->tx_skbuff[i] != NULL) {
			dev_kfree_skb_any(priv->tx_skbuff[i]);
			priv->tx_skbuff[i] = NULL;
		} else if (priv->tx_page && priv->tx_page[i] != NULL) {
...
static void stmmac_dma_operation_mode(struct stmmac_priv *priv)
{
	if (likely((priv->plat->tx_coe) && (!priv->no_csum_insertion))) {
		...
		priv->hw->dma->dma_mode(priv->dma_ioaddr, priv->dma_channel,
					SF_DMA_MODE, SF_DMA_MODE, rfa, rfd);
		tc = SF_DMA_MODE;
	} else
		priv->hw->dma->dma_mode(priv->dma_ioaddr, priv->dma_channel,
					tc, SF_DMA_MODE, rfa, rfd);
}
...
static void stmmac_dma_stop(struct stmmac_priv *priv)
{
	...
	/* TOE notice here !!!
	 * we adjust the sequence of disable MAC and DMA to make sure
	 * one port down does not affect the other port.
	 * Sequence is:
	 * disable MAC rx ---> wait time(N ms) ---> disable DMA rx
	 * ---> disable DMA tx ---> wait time(X ms) ---> disable MAC tx.
	 */

	/* Disable the MAC Rx/Tx */
	stmmac_disable_mac_rx(priv->ioaddr);
	...
	/*  Then disable Rx DMA */
	/* Notice: don't disable RX DMA to make rx more safe */
	...
	/* Disable the MAC Rx/Tx */
	/* Notice: don't disable MAC TX to avoid tx process stopped
	 * when in bonding mode.
	 */
	/* stmmac_disable_mac_tx(priv->ioaddr); */
}
...
#ifdef TNK_BONDING
	if (hitoe)
		tnkhw_bonding_setstatus_slave(priv->id, 0);
#endif
...
static netdev_tx_t stmmac_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	for (i = 0; i < nfrags; i++) {
		...
		desc->des2 = dma_map_page(priv->device, skb_frag_page(frag),
					  frag->page_offset,
					  len, DMA_TO_DEVICE);
		priv->tx_skbuff[entry] = NULL;
		priv->tx_page[entry] = skb_frag_page(frag);
		get_page(skb_frag_page(frag));
#ifdef TNK_BONDING
		/* add for toe bonding about IGMP and ARP data */
		if (hitoe) {
			if (((iph->protocol == IPPROTO_IGMP) &&
				(skb->protocol == htons(ETH_P_IP)))
				|| (skb->protocol == htons(ETH_P_ARP)))
				priv->hw->desc->set_tx_igmp(desc);
			else
				priv->hw->desc->clean_tx_igmp(desc);
		}
#endif
	...
	if (hitoe) {
		if (((iph->protocol == IPPROTO_IGMP) &&
					(skb->protocol == htons(ETH_P_IP)))
				|| (skb->protocol == htons(ETH_P_ARP)))
			priv->hw->desc->set_tx_igmp(first);
		else
			priv->hw->desc->clean_tx_igmp(first);
	}
#endif
		...
		if (unlikely(status == discard_frame))
			...
			if (netif_msg_hw(priv))
				pr_debug("\tdesc: %p [entry %d] buff=0x%x\n",
					 p, entry, p->des2);
#endif
			/* receive packet length too long, drop it */
			if (frame_len > DMA_BUFFER_DESC_SIZE) {
				priv->dev->stats.rx_errors++;
				goto get_next_desc;
			}
...
#if 0
			/* TODO: stmmac logic bug, please fix next version.
			 * Once we use TOE, we must disable stmmac rx checksum
			 * offload, so we choose a different branch
			 * to receive skb.
			 */
			if (!priv->rx_coe || (status == csum_none)) {
#else
...
int stmmac_eth_mac_addr(struct net_device *dev, void *p)
{
...
static int stmmac_associate_phy(struct device *dev, void *data)
{
	...
	if (priv->id)
		phy_mode_param = phy1_mode;
	else
		phy_mode_param = phy0_mode;

	if (phy_mode_param) {
		if (!strncmp(phy_mode_param, "rgmii", strlen("rgmii"))) {
			phy_mode = PHY_INTERFACE_MODE_RGMII;
		} else if (!strncmp(phy_mode_param, "mii", strlen("mii"))) {
			phy_mode = PHY_INTERFACE_MODE_MII;
		} else if (!strncmp(phy_mode_param, "rmii", strlen("rmii"))) {
			phy_mode = PHY_INTERFACE_MODE_RMII;
		} else {
			pr_err("%s: wrong phy mode=%s\n",
				__func__, phy_mode_param);
		}
	}
...
static int tnk_init(void __iomem *base_ioaddr, struct device *dev)
{
	...
	else if (max_connections < CONNECTIONS_MIN_VALUE)
		max_connections = CONNECTIONS_MIN_VALUE;

	/*  Initialise the TNK proc interface */
	ret = tnk_proc_init(hitoe ? max_connections : 0);
	if (unlikely(ret)) {
		pr_err("%s:ERROR: initialising TNK proc interface\n", __func__);
		return -1;
	}
	...
	/* Even in bypass mode, bit ipc_stat_en should be
	 * set to let driver know the payload err packet.
	 */
	writel((1 & TNK_MASK_TRX_CTRL_IPC_STAT_EN)
			<< TNK_OFFSET_TRX_CTRL_IPC_STAT_EN,
			stmmac_base_ioaddr + TNK_REG_DMA_TRX_CTRL);
...
static int stmmac_reset(void)
{
	...
	for (channel = 0; channel < 3; channel++) {
		...
		/* Set Timeout value for mutipacket interrupt */
		writel(0xff, stmmac_base_ioaddr
				+ (channel * 0x100) + DMA_RCV_WDG_ADDR);
...
ndev_alloc_fail:
...
#if 0
		if (hitoe)
			/* TOE logic will checksum skb, there is no
			   need to make gmac checksum */
			priv->rx_coe = 0;
		else
#endif
	...
	if (unlikely(ret < 0)) {
		pr_err("%s: failed to initialise TNK component", __func__);
		pr_err(" (error: %d)\n", ret);
		goto out_tnk_init;
	}
...
static u64 stmmac_dmamask = DMA_BIT_MASK(32);
