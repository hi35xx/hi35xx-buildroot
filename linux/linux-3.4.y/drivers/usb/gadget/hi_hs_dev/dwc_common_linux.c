...
void *__DWC_DMA_ALLOC(void *dma_ctx, uint32_t size, dwc_dma_t *dma_addr)
{
	...
	void *buf = dma_alloc_coherent(dma_ctx,
			(size_t)size, dma_addr, GFP_KERNEL);
#else
	void *buf = dma_alloc_coherent(dma_ctx,
			(size_t)size, dma_addr, GFP_KERNEL);
#endif
...
{
	void *buf = dma_alloc_coherent(NULL,
			(size_t)size, dma_addr, GFP_ATOMIC);
	if (!buf)
		return NULL;
...
void __DWC_DMA_FREE(void *dma_ctx, uint32_t size,
		void *virt_addr, dwc_dma_t dma_addr)
{
...
int DWC_AES_CBC(uint8_t *message, uint32_t messagelen,
		uint8_t *key, uint32_t keylen, uint8_t iv[16], uint8_t *out)
{
	...
	if (tfm == NULL) {
		printk(KERN_ERR "failed to load transform for aes CBC\n");
		return -1;
	}
...
{
	/* printk(KERN_INFO "DWC_WRITE_REG32(0x%x,0x%x)\n ",reg,value); */
	writel(value, reg);
}
		...
		uint32_t clear_mask, uint32_t set_mask)
{
	uint32_t value;
	value = readl(reg);
	/* printk("DWC_READ_REG32(0x%x) value: 0x%x\n",reg,value); */
	writel((value & ~clear_mask) | set_mask, reg);
}

#if 0
void DWC_MODIFY_REG64(uint64_t volatile *reg,
			       ...
			       void *data, int32_t msecs)
{
	...
	int result = wait_event_interruptible_timeout(wq->queue,
			cond(data) || wq->abort, msecs_to_jiffies(msecs));
	if (result == -ERESTARTSYS) {
		wq->abort = 0;
		return -DWC_E_RESTART;
	}
...
static void do_work(struct work_struct *work)
{
	...
	work_container_t *container = container_of(dw,
