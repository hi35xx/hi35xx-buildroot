...
static observer_t *alloc_observer(void *mem_ctx, void *observer,
			char *notification,
...
/* Add an observer to observe the notifier for a particular state, event,
   or notification. */
		     ...
		     dwc_notifier_callback_t callback, void *data)
{
	...
	new_observer = alloc_observer(notifier->mem_ctx, observer,
	...
	DWC_INFO("Added observer %p to notifier %p " \
		"observing notification %s, callback=%p, data=%p",
	...
	DWC_CIRCLEQ_FOREACH(n, &manager->notifiers, list_entry) {
		...
		DWC_CIRCLEQ_FOREACH_SAFE(o, o2, &n->observers, list_entry) {
			if (o->observer == observer) {
				DWC_CIRCLEQ_REMOVE_INIT(&n->observers,
						o, list_entry);
				DWC_INFO("Removing observer %p " \
						"from notifier %p " \
						"watching notification %s:",
						o->observer, n->object,
						o->notification);
...
static void cb_task(void *data)
{
	...
	cb->cb(cb->object, cb->notification,
			cb->observer, cb->notification_data, cb->data);
	dwc_free(cb->mem_ctx, cb);
}

void dwc_notify(dwc_notifier_t *notifier,
		char *notification, void *notification_data)
{
	observer_t *o;
	...
	DWC_CIRCLEQ_FOREACH(o, &notifier->observers, list_entry) {
		...
		if (DWC_STRNCMP(o->notification, notification, len) == 0) {
			cb_data_t *cb_data = dwc_alloc(notifier->mem_ctx,
					sizeof(cb_data_t));
