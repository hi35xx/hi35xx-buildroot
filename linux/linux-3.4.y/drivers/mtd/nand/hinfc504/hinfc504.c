...
static int hinfc504_do_read_retry(struct hinfc_host *host)
{
	...
	for (ix = 1; host->uc_er && ix < host->read_retry->count; ix++) {
		...
		hinfc_write(host, HINFC504_INTCLR_UE | HINFC504_INTCLR_CE,
			HINFC504_INTCLR);
		hinfc_write(host, host->NFC_CON, HINFC504_CON);/* enable ecc */
...
#  endif
	...
	if (flash_dev_ex->read_retry_type != NAND_RR_NONE) {
		...
		if (!host->read_retry) {
			PR_BUG(ERSTR_DRIVER
			"This Nand Flash need to enable the 'read retry' "
			"feature. but the driver dose not offer the feature");
		}
	}
...
#define detect_ecc(_start, _end, _val, _mask) do {\
	...
	for (_i = _start; _i < _end; _i++) {\
		...
		if (host->uc_er) \
			printk(KERN_ERR "\tinvalid ecc error:");\
		else if (_cr_er) \
			printk(KERN_ERR "\tvalid ecc error:");\
		else if (!host->uc_er && !_cr_er) \
			printk(KERN_ERR "\tno ecc error:");\
		printk(KERN_INFO "\t%d%s unit: %d\n",\
	...
	case _2K:
		switch (ecctype) {
		case 0x1:
			printk(KERN_INFO "device is 2K1B\n");
			detect_ecc(0, 8, val(0), 0x3);
			break;
		case 0x2:
			printk(KERN_INFO "device is 2K4B\n");
			detect_ecc(0, 2, val(0), 0x3f);
			break;
		case 0x4:
			printk(KERN_INFO "device is 2K24B\n");
	...
	case _8K:
		switch (ecctype) {
		case 0x4:
			printk(KERN_INFO "device is 8K24B\n");
			for (i = 0, x = 12; i < 8; i += 2, x -= 4)
				detect_ecc(i, i + 2, val(x), 0x1f);
			break;
		case 0x5:
			printk(KERN_INFO "device is 8K40B\n");
			for (i = 0, x = 12; i < 8; i += 2, x -= 4)
				detect_ecc(i, i + 2, val(x), 0x3f);
			break;
		default:
...
int hinfc504_nand_init(struct hinfc_host *host, struct nand_chip *chip)
{
	...
	/* The oobfree area for driver could be selected when ecctype is
	 * 8bits/512byte. We select it to 32byte oobfree.
	 */
