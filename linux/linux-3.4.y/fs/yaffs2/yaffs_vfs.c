...
/* Space holding and freeing is done to ensure we have
 * space available for write_begin/end.
 * For now we just assume few parallel writes and
 * check against a small number.
 * Todo: need to do this with a counter to handle
 * parallel reads better                        ******* */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
static int yaffs_sync_object(struct file *file, loff_t start,
				      ...
				      struct yaffs_obj *obj)
{
	if (inode && obj) {
		...
		/* Check mode against the variant type and
		 * attempt to repair if broken.         */
 ...
 * The object might still exist on disk and is just being thrown out of the
 * cache or else the object has actually been deleted and we're being called
 * via the chain yaffs_delete_inode() -> clear_inode()->yaffs_clear_inode()
 */
						     ...
						     void *data, int silent)
{
	...
	if (yaffs_auto_select && yaffs_version == 1 &&
			WRITE_SIZE(mtd) >= 2048) {
