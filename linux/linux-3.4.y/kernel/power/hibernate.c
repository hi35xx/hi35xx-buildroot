...
char compress_method[16] = "lzma";
#else
char compress_method[16] = "lzo";
#endif
volatile unsigned long in_suspend;

unsigned long snapshot_buf_start = CONFIG_SNAPSHOT_BUF_START;
unsigned long snapshot_buf_size = CONFIG_SNAPSHOT_BUF_SIZE;
...
static ssize_t snapshot_buf_start_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%lx\n", snapshot_buf_start);
}

static ssize_t snapshot_buf_start_store(struct kobject *kobj,
		struct kobj_attribute *attr,
		const char *buf, size_t n)
{
	unsigned long start;
...
power_attr(snapshot_buf_start);

static ssize_t snapshot_buf_size_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%lx\n", snapshot_buf_size);
}

static ssize_t snapshot_buf_size_store(struct kobject *kobj,
		struct kobj_attribute *attr,
		const char *buf, size_t n)
{
	unsigned long size;

	if (sscanf(buf, "%lx", &size) == 1) {
		snapshot_buf_size = size;
		return n;
	}

	return -EINVAL;
}
power_attr(snapshot_buf_size);
#endif
...
static struct attribute * g[] = {
	...
	&snapshot_buf_start_attr.attr,
	&snapshot_buf_size_attr.attr,
#endif
	NULL,
};
