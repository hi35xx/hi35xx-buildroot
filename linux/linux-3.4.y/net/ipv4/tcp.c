...
#ifdef CONFIG_TNK
		if (tnk) {
			/* When we come here, it means all skb
			 * in sk_receive_queue is processed.
			 * sk_receive_queue should be empty if MSG_PEEK not set.
			 * So we check socket state to judge whether it's
			 * TOE accelerated state. If true, we will go to process
			 * TOE receive queue to support SO_RCVLOWAT(MSG_WAITALL)
			 *
			 * This code branch can't be removed because
			 * we should not call sk_wait_data when connection is
			 * TOE accelerated state.
			 */
...
static int tcp_repair_options_est(struct tcp_sock *tp,
		char __user *optbuf, unsigned int len)
{
