...
/*****************************************************************************/
void hisfc300_set_system_clock(struct hisfc300_host *host,
		struct spi_operation *op, int clk_en)
{
	...
	if (readl(host->sysreg + HISFC300_CRG48) != regval)
		writel(regval, (host->sysreg + HISFC300_CRG48));
}
/*****************************************************************************/
void hisfc300_get_best_clock(unsigned int *_clock)
{
	int ix, clk;
