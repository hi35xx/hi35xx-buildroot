...
#include "dwmac_dma.h"
#include "dwmac1000.h"
#include "higmacv300_regmap.h"
#ifdef CONFIG_ARCH_GODARM
#include "higmacv300_godarm.h"
#endif
#include "higmacv300_proc.h"
#define STMMAC_RESOURCE_NAME	"stmmaceth"
#define PHY_RESOURCE_NAME	"stmmacphy"
#define SYNOP_GMAC_IOBASE	CONFIG_HIGMACV300_IOADDR
#define SYNOP_GMAC_IOSIZE	CONFIG_HIGMACV300_IOSIZE
#define SYNOP_SYSTEM_IOBASE	CONFIG_HIGMACV300_SYS_IOADDR
#define SYNOP_SYSTEM_IOSIZE	CONFIG_HIGMACV300_SYS_IOSIZE
#ifdef GMAC_HW_PLATFORM_FPGA
#define SYNOP_GMAC_IRQNUM	74
#else
#define SYNOP_GMAC_IRQNUM	CONFIG_HIGMACV300_IRQNUM
#endif

#define STMMAC_POLL_TIMER    (msecs_to_jiffies(50))

/* This may be referenced in other source files */
struct net_device *stmmac_device_list[NUM_GMACS];
...
#undef GMAC_JUMBO_FRAMES_SUPPORTED
				  ...
				  int speed, int full_duplex, int link_up)
{
	...
	int shift = (priv->id == GMAC0_ID) ? 0 : 16;
	int mask = 0xffff;
...
#ifdef CONFIG_HIGMACV300_DUAL_MAC
...
static void init_dma_desc_rings(struct net_device *dev)
{
	 ...
	 */
#ifdef GMAC_JUMBO_FRAMES_SUPPORTED
...
static netdev_tx_t stmmac_xmit(struct sk_buff *skb, struct net_device *dev)
{
	 ...
	 * to the next instruction, to ensure that the LOGIC will see
...
#define GMAC_INTR_STAT_GMAC0_DATA \
	(GMAC_MASK_INTR_STAT_DMA_CH0 << GMAC_OFFSET_INTR_STAT_DMA_CH0)
#define GMAC_INTR_STAT_GMAC0_CTRL \
	(GMAC_MASK_INTR_STAT_GMAC0 << GMAC_OFFSET_INTR_STAT_GMAC0)
#define GMAC_INTR_STAT_GMAC1_DATA \
	(GMAC_MASK_INTR_STAT_DMA_CH1 << MAC_OFFSET_INTR_STAT_DMA_CH1)
#define GMAC_INTR_STAT_GMAC1_CTRL \
	(GMAC_MASK_INTR_STAT_GMAC1 << GMAC_OFFSET_INTR_STAT_GMAC1)
#define GMAC_INTR_GROUP_GMAC0 \
	(GMAC_INTR_STAT_GMAC0_DATA | GMAC_INTR_STAT_GMAC0_CTRL)
#define GMAC_INTR_GROUP_GMAC1 \
	(GMAC_INTR_STAT_GMAC1_DATA | GMAC_INTR_STAT_GMAC1_CTRL)
...
static irqreturn_t stmmac_interrupt(int irq, void *arg)
{
	...
	uint32_t int_status = readl(stmmac_base_ioaddr + GMAC_REG_INTR_STAT);

	stm_proc.interrupts++;
	/*  GMAC 0 */
	if (int_status & GMAC_INTR_GROUP_GMAC0) {
		stm_proc.gmac[0].interrupts++;
		dev = stmmac_device_list[GMAC0_ID];
		if (!dev)
			...
			if (!priv)
				...
				if (int_status & GMAC_INTR_STAT_GMAC0_DATA)
					stmmac_dma_interrupt(priv);

				if ((int_status & GMAC_INTR_STAT_GMAC0_CTRL)
...
#ifdef CONFIG_HIGMACV300_DUAL_MAC
	/*  GMAC 1 */
	if (int_status & GMAC_INTR_GROUP_GMAC1) {
		...
		if (!dev)
			...
			if (!priv)
				...
				if (int_status & GMAC_INTR_STAT_GMAC1_DATA)
					stmmac_dma_interrupt(priv);

				if ((int_status & GMAC_INTR_STAT_GMAC1_CTRL)
				    && likely(priv->plat->has_gmac))
					priv->hw->mac->
					    host_irq_status((void __iomem *)
							    dev->base_addr);
...
static int stmmac_dvr_remove(struct platform_device *pdev)
{
	...
	writel(0, stmmac_base_ioaddr + GMAC_REG_INTR_EN);
...
static int stmmac_reset(void)
{
	...
	pr_info("\n" "**************************************************\n");
	pr_info("*  HIGMACV300 driver built on %s at %s\n", __DATE__, __TIME__);
	pr_info("**************************************************\n");
...
static int stmmac_dvr_probe(struct platform_device *pdev)
{
	...
	/*  Initialise the GMAC proc interface */
	ret = gmac_proc_init();
	...
	/*  This resets the GMAC, DMA registers */
	stmmac_reset();
...
static struct plat_stmmacphy_data stmmac_phy_private_data[] = {
	...
	[0] = {
		.bus_id = 1,
		.phy_addr = CONFIG_HIGMACV300_PHY0_ID,
		.phy_mask = 0,
	...
	[1] = {
		.bus_id = 1,
		.phy_addr = CONFIG_HIGMACV300_PHY1_ID,
		.phy_mask = 0,
		.interface = PHY_INTERFACE_MODE_RGMII,
	}
#endif
...
MODULE_DESCRIPTION("HIGMACV300 Ethernet driver");
