...
static void hieth_reset(int rst)
{
	...
	hieth_set_regbit(HIETH_SYSREG_BASE + 0x48, rst, 2);
...
static inline void hieth_clk_ena(void)
{
	...
	unsigned long flags;
	local_irq_save(flags);
	val = readl(HIETH_SYSREG_BASE + 0x38);
	val |= ((1 << 26) | (1 << 3));
	writel(val, HIETH_SYSREG_BASE + 0x38);
	val = readl(HIETH_SYSREG_BASE + 0x54);
	val |= (1 << 0);
	writel(val, HIETH_SYSREG_BASE + 0x54);
	local_irq_restore(flags);
}
...
static inline void hieth_clk_dis(void)
{
	...
	unsigned long flags;
	local_irq_save(flags);
	val = readl(HIETH_SYSREG_BASE + 0x3c);
	val |= ((1 << 26) | (1 << 3));
	writel(val, HIETH_SYSREG_BASE + 0x3c);
	val = readl(HIETH_SYSREG_BASE + 0x54);
	val &= ~(1 << 0);
	writel(val, HIETH_SYSREG_BASE + 0x54);
	local_irq_restore(flags);
