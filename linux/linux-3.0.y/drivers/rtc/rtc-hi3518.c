...
static int spi_rtc_write(char reg, char val)
{
	...
	pr_debug("WRITE, reg 0x%02x, val 0x%02x", reg, val);
	...
	do {
		...
		pr_debug("read 0x%x", r_data.u32);
		pr_debug("test busy %d",r_data.bits.spi_busy);
	...
	if (mode == TEMP_SEL_OUTSIDE) {
		...
		if (cnt >= RETRY_CNT) {
			/*maybe temperature capture module is error,
			  need reset */
			pr_err("RTC_TEMP_STAT not ready\n");
			temp_crg_reset();
			goto timer_again;
		}
...
static int hi3518_rtc_open(struct device *dev)
{
	return 0;
}

static void hi3518_rtc_release(struct device *dev)
{
}
...
static const struct rtc_class_ops hi3518_rtc_ops = {
	.open		= hi3518_rtc_open,
	.release	= hi3518_rtc_release,
...
static int __init hi3518_rtc_init(void)
{
	...
	ret = request_irq(HI3518_RTC_IRQ, &rtc_alm_interrupt, 0,
			"RTC Alarm", NULL);
	if(ret) {
...
MODULE_DESCRIPTION("driver for Hi3518 internal RTC");
MODULE_LICENSE("GPL");
